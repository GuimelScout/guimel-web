...
export default function SharedModal({
  ...
  let filteredImages = images?.filter((img: GalleryImageType) =>
          ...
          {loaded && (
            <div className="relative aspect-[3/2] max-h-full w-full">
              ...
              <div className="absolute top-0 right-0 flex items-center gap-2 p-3 text-white">
                ...
                <button
                  onClick={() =>
                    downloadPhoto(currentImage?.image.url || "", `${index}.jpg`)
                  }
                  className="rounded-full bg-black/50 p-2 text-white/75 backdrop-blur-lg transition hover:bg-black/75 hover:text-white"
            ...
            <div className="fixed inset-x-0 bottom-0 z-40 overflow-hidden bg-gradient-to-b from-black/0 to-black/60">
              ...
              >
                <AnimatePresence initial={false}>
                  {filteredImages && filteredImages.map(({ id, image }) => (
                    <motion.button
                      initial={{
                        width: "0%",
                      ...
                      className={`${
                        Number(id) === index
                          ...
                          : "z-10"
                      } ${Number(id) === 0 ? "rounded-l-md" : ""} ${
                       images && Number(id) === images.length - 1 ? "rounded-r-md" : ""

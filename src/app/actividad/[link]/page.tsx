...
import renderers from "@/utils/renderes";
...
const Location = ({ params }: { params: { link: string } }) => {
  ...
  const handleRatingChange = (newRating: number) => {
   ...
   const onSubmit = async (data:any) => {
    ...
    const newReview = {
      activity: {connect: { id: data.activity_id }},
      rating: data.rating,
      review: data.review,
    };
  ...
  const getImageGalleryListing = (): GalleryImageType[] => {
    return (data?.activity.gallery ?? []).map((item: any, index: number) => ({
      id: index.toString(),
  ...
  const newImageUrl = data?.activity.image?.url || "";
  galleryModal.push({
      ...
      <div className="listingSection__wrap !space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <span>
            ...
            <span className="ml-1">  {data?.activity.address}</span>
          </span>
        </div>
      ...
      <div className="listingSectionSidebar__wrap shadow-xl">
        ...
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between text-neutral-6000 dark:text-neutral-300">
            <span>${parseFloat(data?.activity.price || "0.00").toFixed(2)} x {totalGuests} personas</span>
            <span>${parseFloat(data?.activity.price || "0.00") * totalGuests}</span>
          </div>
          {/* <div className="flex justify-between text-neutral-6000 dark:text-neutral-300">
  ...
  const renderSection5 = () => {
    return (
      <div className="listingSection__wrap">
        ...
        <div className="flex items-center space-x-4">
          <Avatar
            hasChecked={data?.activity.hostBy?.verified}
          ...
          <div>
            ...
            <div className="mt-1.5 flex items-center text-sm text-neutral-500 dark:text-neutral-400">
              ...
              <span> {data?.activity.hostBy?.activityCount} actividades </span>
              <span className="mx-2">|</span>
              <span> {data?.activity.hostBy?.lodgingCount} hospedajes</span>
            </div>
          </div>
        </div>
        ...
        <span className="block text-neutral-6000 dark:text-neutral-300">
          {data?.activity.hostBy?.description}
        </span>
        ...
        <div className="block text-neutral-500 dark:text-neutral-400 space-y-2.5">
          <div className="flex items-center space-x-3">
            ...
            <span>
              Se unió en {data?.activity.hostBy?.createdAt 
                ? new Date(data.activity.hostBy.createdAt).toLocaleDateString("es-ES", { year: "numeric", month: "long" }) 
                : "Fecha desconocida"}
            </span>
          </div>
          <div className="flex items-center space-x-3">
  ...
  const renderSection6 = () => {
    return (
      <div className="listingSection__wrap">
        {/* HEADING */}
        <h2 className="text-2xl font-semibold">Reseñas ({data?.activity.reviewCount})</h2>
        <div className="w-14 border-b border-neutral-200 dark:border-neutral-700"></div>
        ...
        <div className="aspect-w-5 aspect-h-5 sm:aspect-h-3 ring-1 ring-black/10 rounded-xl z-0">
          <div className="rounded-xl overflow-hidden z-0">
            <iframe
              ...
              //src={`https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=${data?.activity.address}`}
              src={`https://maps.google.com/maps?q=${encodeURIComponent(data?.activity.address || '')}&t=&z=13&ie=UTF8&iwloc=&output=embed`}
            ></iframe>
          </div>
        </div>
      </div>
        ...
        <header className="rounded-md sm:rounded-xl">
          {(!data?.activity.image || !data?.activity.image.url) && gallery.length === 0 ? (
            <PlaceholderGallery
              ...
              className="w-full"
              showText={true}
              customText="Actividad sin imágenes"
            />
          ) : (
            <div className="relative grid grid-cols-3 sm:grid-cols-4 gap-1 sm:gap-2">
              ...
              >
                <ImageWithPlaceholder
                  ...
                  alt={data?.activity?.name || link}
                  className="object-cover rounded-md sm:rounded-xl"
                  fill
                  sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw"
                  placeholderText="Actividad sin imagen"
                />
